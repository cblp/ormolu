...
p_hsDecls style decls =
  p_layout defaultLayoutOptions (separated decls)
  where
    pDecl = p_hsDecl style

    separated [] = []
    separated (x:[]) = [located x pDecl]
    separated (x:y:xs) =
      let px = located x pDecl
       in if separatedDecls (unLoc x) (unLoc y)
          then (px >> breakpoint') : separated (y:xs)
          else px : separated (y:xs)
